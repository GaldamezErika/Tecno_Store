...
try {
    ...
    //Recipients
    $mail->setFrom('correophpn2@gmail.com', 'Tecno Store');  //Desde donde se enviaran los correos**
    ...
    foreach ($usuario as $u) {
        ...
        $mail->Body    = 'Estimad@ '.$u->nombre. " " .$u->apellido.'!! <br><br>
        Este dia '.date('d-m-Y').' ha solicitado la recuperacion de su cuenta, sus datos son:<br><br>
...
//Llamamos al metodo para actualizar la nueva clave que le sera enviada al usuario en el correo
    ...
    $mail->send();
    redirect('/login_controller/correo/okCorreo','refresh');
} catch (Exception $e) {
    redirect('/login_controller/correo/errorEnviar','refresh');
}
